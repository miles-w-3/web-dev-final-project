...
function SearchComponent() {
  ...
  const [posts] = useState([
    ...
    {
      name: 'Sample Favor',
      description: 'This is a sample favor post. hello',
    ...
    {
      name: 'Sample Favor',
      description: 'This is a sample favor post. bye',
      location: { lat: 34.0522, lng: -118.2437 },
      datePosted: new Date(),
      dateNeeded: new Date(),
      postedBy: 'Jane Doe',
  ...
  const handleGoToSearchResults = async () => {
    if (address) {
      try {
        ...
        navigate(
          `/search?criteria=${address || ''}&keyword=${
            keyword || ''
          }&searchType=${postType || ''}`
        );

        const criteria = new URLSearchParams(window.location.search).get(
          'criteria'
        );
  ...
  useEffect(() => {
    ...
    if (storedURL) {
      navigate(
        `/search?criteria=${storedURL}&keyword=${storedKeyword}&searchType=${storedSearchType}`
      );
    }
  }, []);
